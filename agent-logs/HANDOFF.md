...
## 2026-08-30 · 年度 AI 启动指南

**当前状态**：README 已增加年度定制输入清单和可复制的 AI 提示模板；版本升级为 `0.0.3`，等待用户确认后再提交和推送。

**本轮完成**：
- 明确每年需要提供赛季身份、队伍赛事、官方规则、Scout schema、年度指标、计分映射、状态、场地、Proposal 阶段和验收范围。
- 要求未确定内容标记为 `TBD`，场地图未定时保持空背景，并禁止向 AI 提供密钥。
- 提供可直接填写的年度启动提示模板，默认先规划、确认后修改，且不自行 push 或部署。

**验证**：README 标题、徽章和人类/AI 使用章节结构检查通过；`pnpm typecheck`、`pnpm lint`、22 个测试文件 / 145 项测试、`pnpm build` 和 `git diff --check` 全部通过，三处版本一致为 `0.0.3`。

**风险 / 待办**：指南内容等待用户确认；当前改动未提交、未推送。

---

## 2026-08-30 · 年度 AI 指南易懂化

**当前状态**：README 年度指南已改为面向非代码用户的分层说明；版本升级为 `0.0.4`，等待用户确认后再提交和推送。

**本轮完成**：
- 增加“最少准备 5 项”入口，让资料不完整时也能先开始规划。
- 按基本信息、规则、Scout 数据、指标、计分、状态、场地、Proposal、功能范围和验收逐项解释需要提供什么、为什么需要和怎样提供。
- 增加脱敏 payload 示例、完整填空提示词，以及 AI 在写代码前必须交付的五项检查结果。

**验证**：README 标题、徽章、章节与代码块结构检查通过；`pnpm typecheck`、`pnpm lint`、22 个测试文件 / 145 项测试、`pnpm build` 和 `git diff --check` 全部通过，三处版本一致为 `0.0.4`。
...
**当前状态**：详细年度需求指南已从 README 移到 `SEASON_REQUIREMENTS.md`；README 只保留需求确认入口；版本升级为 `0.0.5`。

**本轮完成**：
- 新建 `SEASON_REQUIREMENTS.md`，集中保存年度资料清单、脱敏样例、完整提示模板和 AI 第一阶段交付要求。
- README 明确要求先通过需求文档确认当年需求，再按 `SEASON_CUSTOMIZATION.md` 实现和验收。
- README 不再承载详细年度需求正文，恢复为简洁仓库入口。

**验证**：README 双向文档链接、单一 H1、徽章和人类/AI 使用章节检查通过；`pnpm typecheck`、`pnpm lint`、22 个测试文件 / 145 项测试、`pnpm build` 和 `git diff --check` 全部通过，三处版本一致为 `0.0.5`。

**风险 / 待办**：文档内容等待用户确认；当前改动未提交、未推送。

---

## 2026-08-30 · 年度需求批注修订

**当前状态**：年度需求文档已改为由人提供 Cyber Scout 代码位置、当年数据和计算逻辑，AI 负责技术整理；机器人状态收敛为正常、未到场、宕机；版本升级为 `0.0.6`。

**本轮完成**：
- 补充 Cyber Scout 文件夹、GitHub 仓库或压缩包三种提供方式，并明确每年必须由人提供的三类材料。
- 默认从 `strategy-template` 最新 commit 开始，解释 Proposal 阶段是比赛阶段，并把验收要求改为业务结果描述。
- 状态类型、归一化、历史值兼容、摘要、徽标和图表统一为三种状态。

**验证**：状态专项 2 个文件 / 8 项测试、全量 22 个文件 / 145 项测试、`pnpm typecheck`、`pnpm lint`、`pnpm build`、版本一致性和 `git diff --check` 全部通过。真实浏览器确认登录页版本为 `0.0.6`，桌面 1280px 与手机 390×844 在浅色/深色下均无横向溢出且控制台无 error / warning；业务页因本地无登录会话未做视觉检查。

**风险 / 待办**：旧通信和故障状态会兼容映射为宕机，无法识别的旧状态映射为正常；当前改动未提交、未推送。

---

## 2026-08-30 · 固定机器人状态判断

**当前状态**：年度需求不再要求每年提供未到场和宕机判断，固定沿用当前 `noShow`、`disabled` 与 20 秒宕机阈值；版本升级为 `0.0.7`。

**本轮完成**：同步修改机器人状态说明和可复制模板，未改动现有判断代码。
...
**当前状态**：本次修复已完成 skill 审查和本地验证，版本为 `0.0.14`；交付目标为 `strategy-template` 与 `strategy-2026`，两分支分别提交并推送，保留各自赛季实现。
...
**验证**：typecheck、lint、158 项测试、build、git diff --check 和三处版本一致性检查通过。使用真实 StrategyBoard 组件和构建 CSS 的隔离页面，在 Chrome 与 WebKit、1280/390px、浅色/深色下验证绘制、撤销、机器人拖动、备注输入和无横向溢出；无页面脚本错误；本地登录页 WebKit 会记录 Vercel Analytics debug 脚本被既有 CSP 拦截，未放宽 CSP。模板未登录跳转和登录页版本检查通过。构建 CSS 包含 user-select 与 -webkit-touch-callout 声明。
