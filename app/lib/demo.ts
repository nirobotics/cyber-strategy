...
export const DEMO_EVENT_NAME = "FIT District Championship - Apollo Division";
...
export function buildDemoData(dataset: ScoutingDataset, matches: CombinedMatch[]) {
  ...
  if (originalTeams.length !== 45) {
    throw new Error(`Apollo Demo 需要 45 支队伍，当前读取到 ${originalTeams.length} 支。`);
  }
  ...
  return {
    dataset: {
      ...dataset,
      id: `demo-${DEMO_EVENT_KEY}`,
      title: `${DEMO_EVENT_NAME} · Demo`,
...
function buildScoutMap(dataset: ScoutingDataset) {
  const names = [...new Set(Object.values(dataset.teamData).flatMap((team) =>
