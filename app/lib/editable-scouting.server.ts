...
export async function saveEditableScoutingRecord({
  ...
  if (superValues && rows.superRow) {
    updates.push(updatePayload(db, rows.superRow.id, {
      ...patchEditableValues(rows.superRow.payload, query.team, "super", superValues),
      strategyUpdatedAt: updatedAt,
      strategyUpdatedBy: updatedBy,
    }));
  }
  ...
  await appendAudit("cyber_scout.record.edit", {
    ...
    changedFields: [
      ...(normal ? editableFieldsFor("normal").map((field) => field.key) : []),
      ...(superValues ? editableFieldsFor("super").map((field) => field.key) : []),
    ],
